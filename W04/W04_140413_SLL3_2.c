...
int main()
{
    ...
    // Iterate until the node before tail
    // Stop when temp->next is tail
    while(temp->next != tail)
    {
        temp = temp->next;
    }

    tail = temp;            // Move tail to the previous node
    temp = tail->next;      // Store the last node
    tail->next = NULL;      // Disconnect last node
    free(temp);             // Free the last node from memory
